...
OUT_DIR  = os.path.join(BASE_DIR, "recommendation_outputs_cf1")
...
def build_cf_scores(user_f, item_f, test_uids):
    ...
    return user_norm @ item_norm.T, np.array(is_warm_list)


def build_cb_scores(test_uids):
    """Content-based score matrix using fused SVD embeddings."""
    ...
    user_mat  = np.array(user_vecs)
    user_norm = user_mat / (np.linalg.norm(user_mat, axis=1, keepdims=True) + 1e-9)
    return user_norm @ emb_norm.T, np.array(is_warm_list)
...
cb_res = evaluate_scores(cb_scores, cb_warm, test_lids, "Content-Based (SVD-256)")
